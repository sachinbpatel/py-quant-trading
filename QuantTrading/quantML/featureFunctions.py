...
def getPrevWeeks(tickerDataRaw):
    ...
    period=20 # This specifies that we are using the last 4 weeks . We take each "week" to be a period of
    # 5 trading days
...
def daysToWeeks(dailyRtns):
    ...
    weekSignals="0"
    for i in range(dailyRtns.size//5):
        weeklyRtn=np.exp(np.sum(dailyRtns[(i*5):i*5+5]))-1
