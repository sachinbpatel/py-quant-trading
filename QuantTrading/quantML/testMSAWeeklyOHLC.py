...
def printSampleDataFrameWeekly(tickerSymbol):
    ...
    tickerDataRaw = getRawDataWeekly(tickerSymbol,start_date,end_date,options)

    closingPrices = tickerDataRaw['AdjClose']
    _3MonthsAvgs = closingPrices.rolling(window=200).mean()
    ...
    candlestick_ohlc(ax, ohlc, width=3.7, colorup='green', colordown='r', alpha=1)
...
tickerSymbol ="SLV"
printSampleDataFrameWeekly(tickerSymbol)
print("done")